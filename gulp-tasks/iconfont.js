...
function iFont() {
  ...
  return src(options.theme.sass + '**/*.svg')
    .pipe(iconfont(options.glyphIconFont))
    .on('glyphs', (glyphs) => {
      for (const i of glyphs.keys()) {
        glyphs[i]['fontSize'] = sizes[i];
      }
      let svgIconFontOptions = options.svgIconFont;
      svgIconFontOptions['glyphs'] = glyphs.map(mapGlyphs);
      return src(options.rootPath.src + 'svg/_svg-font-template.scss')
        .pipe(consolidate('lodash', svgIconFontOptions))
